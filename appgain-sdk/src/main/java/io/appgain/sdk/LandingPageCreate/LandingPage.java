...
public class LandingPage {
    ...
    public void enqueue (final LandingPageResponseListener listener){
        if( AppGain.getApiKey()==null){
            listener.onLandingPageFail(new BaseResponse("404" , " Api key  not found  , make sure u initiate the sdk "));
            return;
        }
        if (AppGain.getAppID() == null){
            listener.onLandingPageFail(new BaseResponse("404" , " App id not found  , make sure u initiate the sdk "));
            return;
        }
        ...
        call.enqueue(new CallbackWithRetry<LandingPageResponse>(call, new onRequestFailure() {
            ...
            public void onFailure(Throwable t) {
                if (listener !=null)
                    listener.onLandingPageFail(new BaseResponse(Config.NETWORK_FAILED, t.getMessage()));
            ...
            public void onResponse(Call<LandingPageResponse> call, Response<LandingPageResponse> response) {
                ...
                if (response.isSuccessful() && response.body()!=null){
                    listener.onLandingPageCreated(response.body());
                }else {
                    if (listener!=null)
                        try {
                            listener.onLandingPageFail(Utils.getAppGainFailure(response.errorBody().string()));
